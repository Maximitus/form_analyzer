import { COCO_INDEX, type ClubHeadEstimate, type Point2D } from '../types/pose';
...
/**
 * Shaft direction in image space: continue past the trail hand (closer to the
 * head on the grip). When the hands stack in a face-on view, blend in "away
 * from the torso" and down.
 */
...
export function landmarksFromCoco(
  xs: ArrayLike<number>,
  ys: ArrayLike<number>,
  ...
  const take = (index: number): Point2D | null => {
    if ((scores[index] ?? 0) < minScore) return null;
    return { x: xs[index], y: ys[index] };
  };
  const leftWrist = take(COCO_INDEX.leftWrist);
  const rightWrist = take(COCO_INDEX.rightWrist);
  const leftHip = take(COCO_INDEX.leftHip);
  const rightHip = take(COCO_INDEX.rightHip);
  const leftShoulder = take(COCO_INDEX.leftShoulder);
  const rightShoulder = take(COCO_INDEX.rightShoulder);
  const leftAnkle = take(COCO_INDEX.leftAnkle);
  const rightAnkle = take(COCO_INDEX.rightAnkle);
  const mid = (a: Point2D | null, b: Point2D | null): Point2D | null =>
    a && b ? { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 } : a ?? b;

  return {
    leadWrist: leadIsLeft ? leftWrist : rightWrist,
    trailWrist: leadIsLeft ? rightWrist : leftWrist,
    midHip: mid(leftHip, rightHip),
    midShoulder: mid(leftShoulder, rightShoulder),
    head: take(COCO_INDEX.nose),
    midAnkle: mid(leftAnkle, rightAnkle),
  };
}
